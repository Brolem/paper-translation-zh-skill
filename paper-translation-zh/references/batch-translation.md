...
Create three part files and one merged file in the source paper's directory:
...
When the paper is too long for one turn, complete the next part file rather than pretending the entire translation is done. In the final response, state which part is complete and which files remain.
